...
public class WebTablePage {
    ...
    public boolean isRecordPresent(String name, String role, String location) {
        ...
        for (WebElement row : rows) {
            List<WebElement> cells = row.findElements(By.tagName("td"));
            if (cells.size() >= 4 &&
                cells.get(1).getText().equalsIgnoreCase(name) &&
        ...
        try {
            ...
            for (WebElement row : rows) {
                ...
                if (cells.size() > 0 && cells.get(1).getText().equalsIgnoreCase(name)) {
                    ...
                    // ✅ Wait until record disappears from DOM (use a fresh locator, not the old row object)
                    wait.until(ExpectedConditions.invisibilityOfElementLocated(
                            By.xpath("//td[text()='" + name + "']/parent::tr")
                    ));
